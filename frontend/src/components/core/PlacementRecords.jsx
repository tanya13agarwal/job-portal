...
const PlacementRecords = () => {
  const handlePlacementRecordsClick = () => {
    window.open(
      "https://www.akgec.ac.in/placements/placement-records/",
      "_blank"
    );
  };
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/dashboard/dashboard-stats`
        );
        setStats(response.data.data);
      } catch (error) {
        console.error("Error fetching placement stats:", error);
  ...
  const preprocessPlacementData = (data) => {
    ...
    const dataMap = new Map(
      data.map((item) => [item._id, item.totalPlaced || item.totalApplications || 0])
    );
  ...
  const processedPlacementData = preprocessPlacementData(
    stats?.studentPlacedPerYear || []
  );
    ...
    <div className="p-4 sm:p-6 md:p-10">
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-col gap-8 items-center">
            {/* Line Chart for Students Placed per Year */}
            <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg w-full md:w-[60%] h-[350px] hover:scale-105 hover:shadow-xl transition-all duration-300">
          ...
          <div className="mt-10 flex flex-col md:flex-row items-center justify-center gap-3">
            <p className="text-center md:text-left">
              Want to see our past Placement Records?
            </p>
            <button
              onClick={handlePlacementRecordsClick}
              className="hover:bg-customDarkBlue text-customDarkBlue rounded hover:text-white px-4 py-2 border border-customDarkBlue transition-all duration-300 hover:scale-105"
