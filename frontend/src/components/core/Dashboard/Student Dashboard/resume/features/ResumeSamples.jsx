...
const ResumeSamples = () => {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 sm:p-6 md:p-8">
      ...
      <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-gray-900 text-center">
        Resume Samples
      </h2>
      ...
      <div className="p-4 md:p-6 border border-gray-200 rounded-lg">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {samples.map((sample) => (
            <div key={sample.id} className="bg-gray-100 p-4 rounded-lg shadow hover:scale-105 transition-all duration-200">
              <img src={sample.image} alt={sample.name} className="w-full h-32 sm:h-40 object-cover mb-4" />
              <h3 className="text-base sm:text-lg font-bold mb-2">{sample.name}</h3>
              <p className="text-sm sm:text-base text-gray-600">Company: {sample.company}</p>
              <a
                href={sample.file}
                target="_blank"
                rel="noopener noreferrer"
