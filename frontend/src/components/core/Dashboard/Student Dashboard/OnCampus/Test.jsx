...
const Test = () => {
  ...
  if (isLoading) {
    return (
      <div className='w-full h-screen flex flex-col items-center justify-center'>
        ...
        <p className='font-semibold text-lg md:text-2xl mt-4 md:mt-6'>Please Wait while the page is loading...</p>
      </div>
    );
  }
  ...
  if (isTestFinished) {
    return (
      <>
        <button onClick={backBtn} className="bg-customDarkBlue text-white w-[40px] md:w-[50px] px-3 py-3 mt-3 rounded-full hover:scale-95 active:scale-105 transition-all duration-200">
          <IoMdArrowRoundBack className="text-lg md:text-2xl" />
        </button>

        <div className="p-6 mx-auto bg-white mt-6 md:mt-10 rounded-lg font mb-10 shadow-lg max-w-[95%] lg:max-w-[75%]">
          <h1 className="text-2xl md:text-3xl font-extrabold my-4 md:my-6 text-center text-customDarkBlue">Test Finished!</h1>

          <div className="my-3 p-4 md:p-6 bg-white border border-gray-300 rounded-lg shadow-sm">
            <p className="text-lg md:text-2xl font-semibold text-customDarkBlue">
              Your Score: <span className="text-green-600">{score} / 30</span>
            </p>
            <p className="text-lg text-gray-600 mt-3">
           Your score reflects your current performance in key placement areas. Keep improving to increase your chances of securing a placement.
          ...
          Based on your score, let's calculate how strong your placement prospects are in top companies.
            ...
            <p className="text-base md:text-lg text-gray-600">
              Your Placement Probability: {placementPercentage !== null ? `${placementPercentage.toFixed(2)}%` : "Calculating..."}
            </p>
          </div>
...
<div className="mt-12 text-start flex flex-col gap-4 p-6 bg-gray-50 border border-gray-200 rounded-lg shadow-sm">
  ...
  return (
    <>
      ...
      <div className="p-4 md:p-6 mx-auto bg-white mt-6 md:mt-10 rounded-lg font mb-10 max-w-[95%] lg:max-w-[75%]">
        <h1 className="text-lg md:text-2xl font-bold mb-4 flex items-center justify-center">Aptitude Mock Test</h1>
        <div className='flex flex-wrap items-center justify-between my-4'>
          <div className="mb-4 bg-transparent border border-customDarkBlue px-4 py-2 rounded-md">
            <span className="text-sm md:text-lg">Question {questionCount + 1} / 30</span>
          </div>
          <div className="mb-4 bg-customDarkBlue text-white px-4 py-2 rounded-md">
            <span className="text-sm md:text-lg">Time Left: {formatTime(timer)}</span>
          </div>
        </div>
        {currentQuestion ? (
          <div>
            <p className="text-base md:text-lg font-semibold mb-4">{currentQuestion.question}</p>
            <div className="mb-4">
              {currentQuestion.options.map((option) => (
                <label
                  key={option}
                  className={`block p-2 rounded text-sm md:text-base mb-2 cursor-pointer ${
                    isAnswerSubmitted
                      ? option === correctAnswer
                        ? 'bg-green-200 border-green-500' // Highlight correct answer in green
                        : option === selectedAnswer && option !== correctAnswer
          ...
          <div className='flex flex-col gap-3 items-center justify-center text-black'>
            <div className='spinner'></div>
            <div className='text-lg md:text-2xl'>Loading Questions...</div>
          </div>
        )}
      </div>
    </>
