    ...
    <div class={`${status === "Published" ? "bg-green-300 text" : "bg-white" } w-full transition-all hover:shadow-xl duration-200  border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700`}>
        ...
        <div onClick={toggleHandler} class="flex  transition-all duration-200 cursor-pointer flex-col items-center px-4 pt-8 pb-10">
            ...
            <div onClick={(e) => e.stopPropagation()} class="flex items-center gap-3 mt-4 md:mt-6">
                <button onClick={btn1Handler} class="px-4 py-2 rounded border border-transparent active:scale-90 text-[#fff] bg-customDarkBlue transition-all duration-200 hover:bg-transparent hover:text-black hover:border-[0.5px] hover:border-customDarkBlue ">{compbtn1}</button>
                <button onClick={btn2Handler} disabled = {disable} href="/test" class={`${disable ? "text-gray-400 bg-gray-200" : "active:scale-90 transition-all duration-200 border border-customDarkBlue hover:bg-customDarkBlue hover:text-[#fff]"} px-4 py-2 rounded `}>{compbtn2}</button>
            </div>
        </div>
        {
